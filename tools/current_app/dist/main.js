...
let launchWorkspace = null;
const folderArg = process.argv.find((arg, index) => {
    if (index < 2) return false;
    return !arg.startsWith('-') && !arg.startsWith('antigravity://') && fs.existsSync(arg) && fs.statSync(arg).isDirectory();
});
if (folderArg) {
    launchWorkspace = folderArg;
}
// ---------------------------------------------------------------------------
// State
// ---------------------------------------------------------------------------
...
electron_1.app.on('second-instance', (event, commandLine) => {
    ...
    if (wins.length > 0) {
        ...
        // Fix: recreate window if none are open and app was running in background
    ...
    const folderArg = commandLine.find((arg, index) => {
        if (index < 2) return false;
        return !arg.startsWith('-') && !arg.startsWith('antigravity://') && fs.existsSync(arg) && fs.statSync(arg).isDirectory();
    });
    if (folderArg) {
        const activeWins = electron_1.BrowserWindow.getAllWindows();
        if (activeWins.length > 0) {
            activeWins[0].webContents.send('open-workspace-folder', folderArg);
        }
    }
});
...
electron_1.app
    ...
    electron_1.ipcMain.handle('get-launch-workspace', () => {
        const ws = launchWorkspace;
        launchWorkspace = null; // Clear after read
        return ws;
    });
