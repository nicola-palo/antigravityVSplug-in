  ...
  function triggerOpenFolder() {
    ...
    var timer = setInterval(function () {
      ...
      for (var i = 0; i < elements.length; i++) {
        ...
        if (text.indexOf('Open Folder') !== -1 || text.indexOf('Apri cartella') !== -1) {
          matches.push(el.tagName + '.' + el.className + ' ("' + text.substring(0, 30).trim() + '")');
          var hasChildMatch = false;
          for (var j = 0; j < el.children.length; j++) {
            var childText = el.children[j].textContent || '';
            if (childText.indexOf('Open Folder') !== -1 || childText.indexOf('Apri cartella') !== -1) {
              hasChildMatch = true;
              break;
            }
          }
